...
import { Download, X, Info, Layers, Zap, RefreshCw, Trash2, Copy, Clock } from 'lucide-react';
          ...
          {!result ? (
            ...
            <>
              ...
              <div className="spi-section">
                ...
                {[
                  { name: 'Source Input', key: 'input' },
                  { name: 'Background Inpainting', key: 'back' },
                  { name: 'Text Masking (EN)', key: 'text_en' },
                  { name: 'Translation Render (VI)', key: 'text_vi' },
                  { name: 'Final Fusion', key: 'fuse' },
                ].map(stage => {
                  const stageUrl = (result.stages as unknown as Record<string, string>)[stage.key];
                  const isDone = !!stageUrl;
                  return (
                    <div 
                      key={stage.key} 
                      className="spi-stage-item"
                      onClick={() => {
                        if (isDone) {
                          window.open(imageUrl(stageUrl), '_blank');
                          setEditHistory(h => [{ icon: '🔍', text: `Inspected stage: ${stage.name}`, time: 'just now' }, ...h.slice(0, 9)]);
                        }
                      }}
                      title={isDone ? "Click to view stage output" : "Stage output not available"}
                    >
