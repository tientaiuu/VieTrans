#!/usr/bin/env python3
"""Smoke test: image upload is rejected without an API key."""
...
def read_response(url: str, image_path: Path, timeout: float) -> tuple[int, str]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify that image upload rejects input.png without an API key."
    )
    parser.add_argument(
        "--base-url",
...
def main() -> int:
    ...
    print(f"Testing without API key: POST {url}")
    print(f"Image: {image_path}")
    try:
        status, body = read_response(url, image_path, args.timeout)
    ...
    if status == 401:
        print("PASS: Request without API key was rejected.")
        return 0

    print(f"FAIL: Expected HTTP 401, got HTTP {status}.", file=sys.stderr)
    return 1
