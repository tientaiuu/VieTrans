...
class DebackPipeline:
    ...
    def run_inference(self, input_img_path: str, output_dir: str) -> str:
        ...
        debug_path   = os.path.join(debug_dir, "09_pipeline_debug.json")
        ...
        with open(ocr_path, "w", encoding="utf-8") as f:
            f.write(ocr_text)
        timings["ocr_seconds"] = time.perf_counter() - stage_start
        counts["ocr_regions"] = len(regions)
        counts["translatable_regions"] = len(translatable_regions)
        ...
        timings["translation_seconds"] = time.perf_counter() - stage_start
        ...
        counts["changed_regions"] = len(changed_regions)
        counts["unchanged_regions"] = len(unchanged_regions)
        translation_records = [
            {
                "index": idx,
                "source_text": en_t,
                "translated_text": vi_t,
                "keep_original": _should_keep_original_region(en_t, vi_t),
                "region": {
                    "confidence": region.get("detector_confidence"),
                    "box": region.get("box"),
                },
            }
            for idx, (region, en_t, vi_t) in enumerate(zip(translatable_regions, en_texts, vi_texts))
        ]
        ...
        counts["inpaint_mask_pixels"] = int(np.count_nonzero(mask))
        counts["inpaint_mask_bbox_fill_ratio"] = round(
            counts["inpaint_mask_pixels"] / max(1, mask_bbox_area), 4
        )
        ...
        timings["inpainting_seconds"] = time.perf_counter() - stage_start
        ...
        timings["rendering_seconds"] = time.perf_counter() - stage_start
        ...
        timings["qa_seconds"] = time.perf_counter() - stage_start
        timings["total_seconds"] = time.perf_counter() - total_start
        counts["final_ocr_regions"] = len(final_ocr_regions)
        debug_payload = {
            "counts": counts,
            "timings": timings,
            "qa": qa_report,
            "translation_records": translation_records,
            "render_records": translation_records,
        }
